...
use Carbon\Carbon;
    ...
    function ambil_semuadata($tabel) {
        if (Schema::hasColumn($tabel,'deleted')) {
            $data = DB::table($tabel)->orderBy('id', 'asc')->where('deleted','<>',1)->get();
        }
        else {
    ...
    function ambil_datano($tabel,$request) {
        $data = DB::table($tabel)
          ->select(DB::raw('max(RIGHT(id, 1)) as result'))
          ->where('jenis_asuransi',$request)
          ->get();
          
        return ($data);
    }

    function ambil_semuadatatgl($tabel) {
        $start_date = Carbon::parse(request()->start_date)->toDateString();        
        $end_date = Carbon::parse(request()->end_date)->toDateString();
        $starts_date = $start_date. ' 00:00:01';
        $ends_date = $end_date. ' 23:59:59';
        $data = DB::table($tabel)->orderBy('id', 'asc')->where('deleted','<>',1)->whereBetween('created_time',[$starts_date,$ends_date])->get();        
        return ($data);
    }
