...
<!--End Modal-->
      ...
      <form action="{{route('tindakan.import')}}" method="post" enctype="multipart/form-data">
            ...
            <div class="input-group mb-3 row">
              ...
              <p>untuk file template Tindakan bisa di download disini <a href="{{ url('/storage/public/data-template-import-tindakan.xlsx') }}"> Data Tindakan CSV Excel Import </a> </p>
              </div>
    ...
    <div class="card shadow mb-4">
        <div class="card-header d-sm-flex align-items-center justify-content-between py-3">               
            <h6 class="m-0 font-weight-bold text-primary">Daftar Fasilitas tindakan</h6>
            <a href="{{route('tindakan.export')}}" class="d-sm-inline-block btn btn-primary btn-sm shadow-sm">
            <i class="fas fa-plus fa-sm"></i> Export Data Tindakan</a> 
            <a href="{{route('tindakan.tambah')}}" class="d-none d-sm-inline-block btn btn-primary btn-sm shadow-sm">
            <i class="fas fa-plus fa-sm"></i> Tambah tindakan</a> 
        </div>
            <div class="card-body">
                ...
                <table class="table table-bordered table-striped" id="dataTable" data-order='[[ 0, "desc" ]]' width="100%" cellspacing="0">
                  ...
                  @foreach ($tindakans as $tindakan)
                    <tr>
                      ...
                      <td>
                        <a href ="{{ route('tindakan.edit', $tindakan->id) }}" class="btn btn-sm btn-icon-split btn-warning">
                            <span class="icon"><i class="fa fa-pen" style="padding-top: 4px;"></i></span><span class="text">Edit</span>
                        </a>
                        <a href="javascript:;" data-toggle="modal" onclick="deleteData({{$tindakan->id}})" data-target="#DeleteModal" class="{{Auth::user()->profesi == "Staff" ? 'disabled': ''}} btn btn-sm btn-icon-split btn-danger">
          ...
          <div class="col-sm-6">
             <a href="{{route('tindakan.cetakpdf')}}" id="print2" data-id="" class="btn btn-secondary btn-block">
              <span class="icon"><i class="fa  fa-print" ></i></span><span class="text"> Print Data</span></a>
          </div>
          <div class="col-sm-6">                                    
              <a href="{{route('tindakan.cetakpdf')}}" class="btn btn-primary btn-block">
